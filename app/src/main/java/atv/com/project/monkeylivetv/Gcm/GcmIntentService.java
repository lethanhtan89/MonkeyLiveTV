package atv.com.project.monkeylivetv.Gcm;
...
public class GcmIntentService extends GcmListenerService {
    ...
    public void onMessageReceived(String from, Bundle data) {
        ...
        if(code.equalsIgnoreCase("2001") && MonkeyLive.getBooleanPreference(MonkeyLive.SHOW_NEW_STREAM_NOTIFICATION, true)){
            showNewStreamNotification(data);
        } else if(code.equalsIgnoreCase("2002")) {
            int count = MonkeyLive.getIntPreference(MonkeyLive.USER_FOLLOWERS_COUNT, 0);
            MonkeyLive.setIntPreferenceData(MonkeyLive.USER_FOLLOWERS_COUNT, count + 1);
            if(MonkeyLive.getBooleanPreference(MonkeyLive.SHOW_FOLLOW_NOTIFICATION, true)) {
