...
import sdk.event.CameraOpenedEvent;
import sdk.view.GLCameraEncoderView;
import sdk.view.GLCameraView;
...
//        mSessionConfig = checkNotNull(config);
     ...
     * @param slug
     */
    public void requestThumbnail(int scaleFactor, String slug) {
        uploadSlug = slug;
    ...
    private void saveFrameAsImage() {
        try {
//            File recordingDir = File.createTempFile("Viewora-", "-upload.jpeg");
            File imageFile = File.createTempFile("Viewora-", "-upload.jpeg");;
            mInputWindowSurface.saveFrame(imageFile, mThumbnailScaleFactor, uploadSlug);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void takePicture(final CameraPictureCallBack cameraPictureCallBack) {
        try {
            if (mCamera != null) {
                ...
                for(Camera.Size aSize : supportedSizes){
                    if(aSize.width == 720){
                        cameraWidth = aSize.width;
                        cameraHeight = aSize.height;
                    }
                }
                ...
                mCamera.takePicture(null, null, new Camera.PictureCallback() {
                    ...
                    public void onPictureTaken(byte[] data, Camera camera) {
                        cameraPictureCallBack.onShutter(data);
                    }
                });
            }
        } catch (Exception e){
